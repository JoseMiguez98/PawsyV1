---
name: git-flow
description: Executes the pawsy per-ticket git flow — branch, conventional commits, PR opening, and automatic Linear state transitions via the Linear MCP. Use whenever an agent is working a Linear ticket from `dmitry` team (DMI-XXX). Enforces the rules in AGENTS.md.
---
...
- You're at the repo root of `pawsy-`, on a clean `main` (or on a worktree — see §5).
...
| Ticket | Branch |
|---|---|
| DMI-27 "Crear AGENTS.md con protocolo de loop agentic" | `feature/DMI-27` |
| DMI-8 "Carga de ROM local por el usuario" | `feature/DMI-8` |
| DMI-21 "Servidor de signaling" | `feature/DMI-21` |
...
git checkout main
git pull --ff-only
git checkout -b feature/DMI-XXX
```
